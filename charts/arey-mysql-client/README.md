...
1. Install chart using
   ...
   Example:
    ...
    ```console
    helm install my-arey-mysql-client senzing/arey-mysql-client
    ```

    The command deploys `arey-mysql-client` on the Kubernetes cluster in the default configuration.
...
1. Uninstall/delete the deployment using
   ...
   Example:
    ...
    ```console
    helm uninstall my-arey-mysql-client
    ```
...
The above parameters map to the env variables defined in [bitnami/arey-mysql-client](http://github.com/bitnami/bitnami-docker-%%CHART_NAME%%). For more information please refer to the [bitnami/arey-mysql-client](http://github.com/bitnami/bitnami-docker-%%CHART_NAME%%) image documentation.
...
The above command sets the arey-mysql-client administrator account username and password to `admin` and `password` respectively. Additionally, it sets the MariaDB `root` user password to `secretpassword`.
...
```console
helm install my-arey-mysql-client -f values.yaml senzing/arey-mysql-client
```
