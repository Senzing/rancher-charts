...
1. Install chart using
   ...
   Example:
    ...
    ```console
    helm install my-senzing-configurator senzing/senzing-configurator
    ```

    The command deploys `senzing-configurator` on the Kubernetes cluster in the default configuration.
...
1. Uninstall/delete the deployment using
   ...
   Example:
    ...
    ```console
    helm uninstall my-senzing-configurator
    ```
...
The above parameters map to the env variables defined in [bitnami/senzing-configurator](http://github.com/bitnami/bitnami-docker-%%CHART_NAME%%). For more information please refer to the [bitnami/senzing-configurator](http://github.com/bitnami/bitnami-docker-%%CHART_NAME%%) image documentation.
...
The above command sets the senzing-configurator administrator account username and password to `admin` and `password` respectively. Additionally, it sets the MariaDB `root` user password to `secretpassword`.
...
```console
helm install my-senzing-configurator -f values.yaml senzing/senzing-configurator
```
