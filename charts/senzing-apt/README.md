...
1. Install chart using
   ...
   Example:
    ...
    ```console
    helm install my-senzing-apt senzing/senzing-apt
    ```

    The command deploys `senzing-apt` on the Kubernetes cluster in the default configuration.
...
The above parameters map to the env variables defined in [bitnami/senzing-apt](http://github.com/bitnami/bitnami-docker-%%CHART_NAME%%). For more information please refer to the [bitnami/senzing-apt](http://github.com/bitnami/bitnami-docker-%%CHART_NAME%%) image documentation.
...
The above command sets the senzing-apt administrator account username and password to `admin` and `password` respectively. Additionally, it sets the MariaDB `root` user password to `secretpassword`.
...
```console
helm install my-senzing-apt -f values.yaml senzing/senzing-apt
```
