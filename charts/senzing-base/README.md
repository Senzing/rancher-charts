...
1. Install chart using
   ...
   Example:
    ...
    ```console
    helm install my-senzing-base senzing/senzing-base
    ```

    The command deploys `senzing-base` on the Kubernetes cluster in the default configuration.
...
1. Uninstall/delete the deployment using
   ...
   Example:
    ...
    ```console
    helm uninstall my-senzing-base
    ```
...
The above parameters map to the env variables defined in [bitnami/senzing-base](http://github.com/bitnami/bitnami-docker-%%CHART_NAME%%). For more information please refer to the [bitnami/senzing-base](http://github.com/bitnami/bitnami-docker-%%CHART_NAME%%) image documentation.
...
The above command sets the senzing-base administrator account username and password to `admin` and `password` respectively. Additionally, it sets the MariaDB `root` user password to `secretpassword`.
...
```console
helm install my-senzing-base -f values.yaml senzing/senzing-base
```
