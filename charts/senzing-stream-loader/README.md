...
1. Install chart using
   ...
   Example:
    ...
    ```console
    helm install my-senzing-stream-loader senzing/senzing-stream-loader
    ```

    The command deploys `senzing-stream-loader` on the Kubernetes cluster in the default configuration.
...
1. Uninstall/delete the deployment using
   ...
   Example:
    ...
    ```console
    helm uninstall my-senzing-stream-loader
    ```
...
The above parameters map to the env variables defined in [bitnami/senzing-stream-loader](http://github.com/bitnami/bitnami-docker-%%CHART_NAME%%). For more information please refer to the [bitnami/senzing-stream-loader](http://github.com/bitnami/bitnami-docker-%%CHART_NAME%%) image documentation.
...
The above command sets the senzing-stream-loader administrator account username and password to `admin` and `password` respectively. Additionally, it sets the MariaDB `root` user password to `secretpassword`.
...
```console
helm install my-senzing-stream-loader -f values.yaml senzing/senzing-stream-loader
```
