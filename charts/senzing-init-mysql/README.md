...
1. Install chart using
   ...
   Example:
    ...
    ```console
    helm install my-senzing-init-mysql senzing/senzing-init-mysql
    ```

    The command deploys `senzing-init-mysql` on the Kubernetes cluster in the default configuration.
...
1. Uninstall/delete the deployment using
   ...
   Example:
    ...
    ```console
    helm uninstall my-senzing-init-mysql
    ```
...
The above parameters map to the env variables defined in [bitnami/senzing-init-mysql](http://github.com/bitnami/bitnami-docker-%%CHART_NAME%%). For more information please refer to the [bitnami/senzing-init-mysql](http://github.com/bitnami/bitnami-docker-%%CHART_NAME%%) image documentation.
...
The above command sets the senzing-init-mysql administrator account username and password to `admin` and `password` respectively. Additionally, it sets the MariaDB `root` user password to `secretpassword`.
...
```console
helm install my-senzing-init-mysql -f values.yaml senzing/senzing-init-mysql
```
