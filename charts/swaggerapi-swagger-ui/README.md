...
1. Install chart using
   ...
   Example:
    ...
    ```console
    helm install my-swaggerapi-swagger-ui senzing/swaggerapi-swagger-ui
    ```

    The command deploys `swaggerapi-swagger-ui` on the Kubernetes cluster in the default configuration.
...
1. Uninstall/delete the deployment using
   ...
   Example:
    ...
    ```console
    helm uninstall my-swaggerapi-swagger-ui
    ```
...
The above parameters map to the env variables defined in [bitnami/swaggerapi-swagger-ui](http://github.com/bitnami/bitnami-docker-%%CHART_NAME%%). For more information please refer to the [bitnami/swaggerapi-swagger-ui](http://github.com/bitnami/bitnami-docker-%%CHART_NAME%%) image documentation.
...
The above command sets the swaggerapi-swagger-ui administrator account username and password to `admin` and `password` respectively. Additionally, it sets the MariaDB `root` user password to `secretpassword`.
...
```console
helm install my-swaggerapi-swagger-ui -f values.yaml senzing/swaggerapi-swagger-ui
```
