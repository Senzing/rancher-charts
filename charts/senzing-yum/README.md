...
1. Install chart using
   ...
   Example:
    ...
    ```console
    helm install my-senzing-yum senzing/senzing-yum
    ```

    The command deploys `senzing-yum` on the Kubernetes cluster in the default configuration.
...
The above parameters map to the env variables defined in [bitnami/senzing-yum](http://github.com/bitnami/bitnami-docker-%%CHART_NAME%%). For more information please refer to the [bitnami/senzing-yum](http://github.com/bitnami/bitnami-docker-%%CHART_NAME%%) image documentation.
...
The above command sets the senzing-yum administrator account username and password to `admin` and `password` respectively. Additionally, it sets the MariaDB `root` user password to `secretpassword`.
...
```console
helm install my-senzing-yum -f values.yaml senzing/senzing-yum
```
