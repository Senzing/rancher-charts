...
1. Install chart using
   ...
   Example:
    ...
    ```console
    helm install my-confluentinc-cp-kafka senzing/confluentinc-cp-kafka
    ```

    The command deploys `confluentinc-cp-kafka` on the Kubernetes cluster in the default configuration.
...
1. Uninstall/delete the deployment using
   ...
   Example:
    ...
    ```console
    helm uninstall my-confluentinc-cp-kafka
    ```
...
The above parameters map to the env variables defined in [bitnami/confluentinc-cp-kafka](http://github.com/bitnami/bitnami-docker-%%CHART_NAME%%). For more information please refer to the [bitnami/confluentinc-cp-kafka](http://github.com/bitnami/bitnami-docker-%%CHART_NAME%%) image documentation.
...
The above command sets the confluentinc-cp-kafka administrator account username and password to `admin` and `password` respectively. Additionally, it sets the MariaDB `root` user password to `secretpassword`.
...
```console
helm install my-confluentinc-cp-kafka -f values.yaml senzing/confluentinc-cp-kafka
```
