...
1. Install chart using
   ...
   Example:
    ...
    ```console
    helm install my-microsoft-mssql-tools senzing/microsoft-mssql-tools
    ```

    The command deploys `microsoft-mssql-tools` on the Kubernetes cluster in the default configuration.
...
1. Uninstall/delete the deployment using
   ...
   Example:
    ...
    ```console
    helm uninstall my-microsoft-mssql-tools
    ```
...
The above parameters map to the env variables defined in [bitnami/microsoft-mssql-tools](http://github.com/bitnami/bitnami-docker-%%CHART_NAME%%). For more information please refer to the [bitnami/microsoft-mssql-tools](http://github.com/bitnami/bitnami-docker-%%CHART_NAME%%) image documentation.
...
The above command sets the microsoft-mssql-tools administrator account username and password to `admin` and `password` respectively. Additionally, it sets the MariaDB `root` user password to `secretpassword`.
...
```console
helm install my-microsoft-mssql-tools -f values.yaml senzing/microsoft-mssql-tools
```
